...
def print_log(msg, log_type="info"):  # 提示
    if log_type == 'info':
...
def op_gt(column, condtion_val):  # 判断>
    """
    :param column: eg.age
...
def op_lt(column, condtion_val):  # 判断<
    matched_records = []
    for index, val in enumerate(STAFF_DATA[column]):  # "age": [22,445,2,5,2]
        if float(val) < float(condtion_val):  # 匹配上了
            record = []
...
def op_like(column, condtion_val):  # 判断入职时间
    matched_records = []
    for index, val in enumerate(STAFF_DATA[column]):  # "age": [22,445,2,5,2]
