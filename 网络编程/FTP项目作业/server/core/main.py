...
class FTPServer(object):
    ...
    STATUS_CODE = {
        ...
        201: "Worng username or password!",
        ...
        402: "File exist,but file size dosn't match!"
    }
    ...
    def __init__(self, management_instance):
        ...
        self.sock.bind((settings.HOST, settings.PORT))  # 地址，端口
    ...
    def handle(self):
        """处理与用户的所有指令交互"""
        while True:
            raw_data = self.request.recv(self.MSG_SIZE)  # 拿到消息
            print('------>', raw_data)
            if not raw_data:
                print("connection %s is lost ..." % (self.addr,))
                del self.request, self.addr
                break
            data = json.loads(raw_data.decode("utf-8"))  # 解析消息
    ...
    def authenticate(self, username, password):
        ...
        if username in self.accounts:
            ...
            # print("worng username or password 2")
            return False
    ...
    def _auth(self, data):  # _开头，都是与客户端交互的指令
        """处理用户认证请求"""
        print("auth", data)
    ...
    def _re_get(self, data):
        ...
        full_path = os.path.join(self.user['home'], abs_filename.strip("\\"))
    ...
    def _put(self, data):
        ...
        1.拿到local文件名+大小
        2.检查本地是否有相应的文件。self.user_current_dir/ocal_file
            2.1 if file exist,create a new file with file.tiemstamp suffix.
            2.2 if ot ,create a new file named local_file name
    ...
    def _cd(self, data):
        ...
        1.把target_dir跟user_current_dir拼接
        2.检测 要切换的目录是否存在
